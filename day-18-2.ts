...
function magnitud(s: string): number {
    if (isNaN(parseInt(s))) {
        ...
        let mag1 = magnitud(pair[0])
        let mag2 = magnitud(pair[1])
        return (3 * mag1 + 2 * mag2)
    } else {
        return parseInt(s)
    }
}
...
function iterativeReduce(s: string) {
    let result = s
...
function resolve() {
    ...
    for (let i = 0; i < input.length; i++) {
        for (let j = 0; j < input.length; j++) {
            if (i != j) {
                let result = sum(input[i], input[j])
                result = iterativeReduce(result)
                let m = magnitud(result)
                if (m > maxMagnitud) {
                    maxMagnitud = m
                }
            }
